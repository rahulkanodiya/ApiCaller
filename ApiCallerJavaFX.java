...
public class ApiCallerJavaFX extends Application {
    ...
    private void onApiSubmit() {
        try {
            ...
            // Extract "ID" field from JSON response and set it in ECE_ID field
            String eceId = extractEceId(responseArea.getText());
            eceIdField.setText(eceId);
        } catch (Exception ex) {
    ...
    private String extractEceId(String jsonResponse) {
        try {
            // Parse JSON response
            ...
            return "Error extracting ECE_ID: " + e.getMessage();
        }
    }
}